...
**Đề xuất bổ sung (chưa có trong repository):** nếu muốn chuẩn hóa triển khai bằng container, có thể thêm `docker-compose.yml` gồm 3 service `mysql` (mount sẵn `reliefhub_database.sql` + `seed_data.sql` vào `/docker-entrypoint-initdb.d`), `backend` (build từ `backend/Dockerfile`, phụ thuộc `mysql` healthy) và `frontend` (build từ `frontend/Dockerfile`, phục vụ qua Nginx). Các biến `SPRING_DATASOURCE_URL`, `SPRING_DATASOURCE_PASSWORD`, `JWT_SECRET` nên đưa ra `.env` ngoài repository.

---

## 🔐 Bảo mật — lưu ý trước khi public repository
...
Các số liệu/tuyên bố trong bản README mẫu **không khớp** với mã nguồn trong gói này. Bảng dưới đây là kết quả đo trực tiếp, nên cập nhật lại cho thống nhất:
...
Repository hiện **chưa kèm file `LICENSE`**. Nếu công khai dự án, nên bổ sung giấy phép phù hợp (ví dụ MIT) để làm rõ quyền sử dụng.
